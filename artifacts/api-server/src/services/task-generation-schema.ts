...
async function runTaskGenerationSchemaEnsure() {
  ...
  await db.execute(sql`
    ALTER TABLE task_generation_batches
    ADD COLUMN IF NOT EXISTS source_task_id integer
  `);
  ...
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_task_generation_batches_source_task ON task_generation_batches(source_task_id)`);
